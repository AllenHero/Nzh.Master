...
$(function () {
    ...
    var columnsArray = [
        ...
        { "data": 'SendTime', "title": "发送时间", "searchable": false, "width": "120px", "visible": true, "sortable": true, "render": functionFormatSendTime },
    ...
    var datatableSetting = {
        ...
        "scrollX": true,
        "ajax": {
            "url": "/api/Self/My/EditMyCreatedTask",
            "type": "POST",
    ...
    $('#dataList tbody').on('click', 'a#EditRow', function () {
        ...
        $.ajax({
            url: '/api/andon2/flowable/GetHistoryInstance?id=' + data.SourceId,
...
function GetMyLaunch() {
    ...
    $.ajax({
        type: 'get',
        url: '/api/Self/My/GetMyCreatedTask?rows=' + rows + '&page=' + page + '&SourceModel=' + SourceModel + '&Message=' + Message + '&DateTime=' + DateTime + '&TenantId=' + TenantId + '&id=' + UserId + '&IsEnded=' + IsEnded,
        async: true,
...
function EditView(ID) {
    $.ajax({
        url: '/api/andon2/flowable/GetHistoryInstance/' + ID,
        success: function (value) {
            if (value.status) {
                //初始化页面任务详情
                ExceptionDetail(value.rows);
