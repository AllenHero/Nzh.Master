(function ($, window, document, undefined) {
    ...
    let ComboTree = function (element, options) {
        ...
        this.noresults = null;
        this.dataSource(defaults.url, defaults.methord);
        //this.init();
    };

    ComboTree.prototype.dataSource = function (url, method) {
        if (url) {
    ...
    ComboTree.prototype.setData = function (data) {
        if (data) {
    ...
    ComboTree.prototype.initevent = function () {
        let _this = this;
        //var el = document.getElementById("queryCustomer").getBoundingClientRect()
        //$("#queryCustomer").css("top", (el.top + el.height));
        //点击输入框时候
        window.addEventListener("scroll",function () {
            var el = _this.element.getBoundingClientRect();
            $(_this.element).find('.drop-down-wrap').css("top", (el.top + el.height));
        });
        $(this.element).on('click', function (event) {
            event = event || window.event;

            var el = this.getBoundingClientRect();
            var elDropDown = $(_this.element).find('.drop-down-wrap');
            ...
            if (($(event.target).hasClass('imitationSelect') || $(event.target).hasClass('fa')) && closet.length > 0) {
                ...
                elDropDown.toggle();
                elDropDown.css("top", (el.top + el.height));
    ...
    ComboTree.prototype.initdom = function () {
        $(this.element).children(".input-keyword-wrap").remove();
        $(this.element).children(".drop-down-wrap").remove();
        $(this.element).append(
            '<div class="input-keyword-wrap">' +
            '<div _id="role_select" class="select-menu-input imitationSelect role_select">'+
            '<span _id="myplaceholder" class="input-tips">点击选择</span>'+
            '</div>' +
    ...
    ComboTree.prototype.createitem = function (SampleJSONData, container) {
        for (let j = 0; j < SampleJSONData.length; j++) {
            ...
            var oliCode = SampleJSONData[j].code;
            var oliId = SampleJSONData[j].id;
            var spc = SampleJSONData[j].spec;
            var className = SampleJSONData[j].className;
            // li容器
            let item = $('<li data-name="' + oliName + '" data-id="' + oliId + '" data-code="' + oliCode + '"  data-spc="' + spc + '" data-className="' + className +'"></li>'),
                divitem = $('<div data-name="' + oliName + '" data-id="' + oliId + '" data-code="' + oliCode + '"  data-spc="' + spc + '" data-className="' + className +'" class="title-container"></div>');
    ...
    ComboTree.prototype.checkrow = function (target, oliId, ischeckbox) {
        ...
        oliCode = target[1] ? target[1].getAttribute("data-code"):target.context.parentElement.getAttribute("data-code");
        ...
        let item = $("<span data-id='" + oliId + "' data-code='" + oliCode + "' class='input-keyword-item'></span>"),
        ...
        $("#modal_new_add #txtproductName").attr("value", target.attr('data-code'));
        if (target.attr('data-spc') != null) {
            $("#modal_new_add #txttxtproductSpec").attr("value", target.attr('data-spc'));
        }
        else {
            $("#modal_new_add #txttxtproductSpec").attr("value", "");
        }

        if (target.attr('data-className') != null) {
            $("#modal_new_add #txttxtproductType").attr("value", target.attr('data-className'));
        }
        else {
            $("#modal_new_add #txttxtproductType").attr("value", "");
        }
        _this.roleSelect.append(item);
    };

    ComboTree.prototype.setSelect = function (item) {
        var id = item.attr("data-id");
        if (id) this.checkrow(item, id, true);
    };
    ...
    ComboTree.prototype.clear = function (item) {
        $(this.element).find('.input-keyword-item').remove();
    };
    /**
     * 清空搜索输入框里面的内容
     */
    ...
    $.fn[comboTreeDetail] = function (options) {
        var ctArr = [];
        this.each(function () {
            if (!$.data(this, 'plugin_' + comboTreeDetail)) {
                $.data(this, 'plugin_' + comboTreeDetail, new ComboTree(this, options));
                ctArr.push($(this).data()['plugin_' + comboTreeDetail]);
            }
        });
